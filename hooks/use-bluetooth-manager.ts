...
import { TrackedDevice, getNextColor } from "../utils/bluetooth-helpers";
...
export const useBluetoothManager = () => {
  ...
  const { getCurrentLocationAndAddress, sendLostNotification } =
    useLocationAndNotification();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved) {
        const parsedDevices: TrackedDevice[] = JSON.parse(saved);
        // MIGRASI: Jika data lama belum punya warna, beri warna sekarang
        const migratedDevices = parsedDevices.map((d, index) => ({
          ...d,
          color: d.color || getNextColor(index),
        }));
        setMyDevices(migratedDevices);
      }
  ...
  const startScan = async () => {
    ...
    if (state !== "PoweredOn") {
      Alert.alert("Bluetooth Mati", "Nyalakan Bluetooth dulu.");
      return;
    }
    setScannedDevices([]);
    setIsScanning(true);
    manager.startDeviceScan(
      ...
      (error, device) => {
        ...
        const displayName =
          device.name || (device as any).localName || "Unknown Device";
  ...
  const addDevice = (device: Device) => {
    ...
    setIsScanning(false);
    // Pilih warna berdasarkan jumlah device saat ini
    const assignedColor = getNextColor(myDevices.length);
    const newTracked: TrackedDevice = {
      ...
      status: "DISCONNECTED",
      deviceObject: null,
      color: assignedColor, // <-- SIMPAN WARNA PERMANEN
    };
  ...
  const connectToDevice = async (trackItem: TrackedDevice) => {
    ...
    try {
      ...
      const connectPromise = (async () => {
        ...
        const dev = await manager.connectToDevice(trackItem.id, {
          autoConnect: false,
        });
  ...
  const handleDeviceLost = async (item: TrackedDevice) => {
    ...
    if (locData) {
      sendLostNotification(
        item.name || "Perangkat",
        locData.address || "Lokasi tidak diketahui"
      );
      Alert.alert(
        "Koneksi Terputus!",
        `Perangkat ${item.name} terputus.\nLokasi: ${locData.address}`
      );
    } else {
